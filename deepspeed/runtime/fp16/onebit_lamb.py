                ...
                if self.adam_freeze_key is False:
                    ...
                    if 'non_freeze' in group.keys() and group['non_freeze'] is True:
                        ...
                        if self.extra_stats == 1:
                            self.error_worker_norms.append(
                                torch.norm(state['worker_error']).item())
                            self.error_server_norms.append(
                                torch.norm(state['server_error']).item())
                        if self.initialize and self.compress_mode == 0:
                            grad_recover = (exp_avg - exp_avg_back * beta1) / (1 - beta1)
                            exp_avg_sq_back.mul_(beta2).addcmul_(
                                1 - beta2,
                                grad_recover,
                                grad_recover)
                    # if self.initialize:
                ...
                if self.initialize:
                    ...
                    if self.extra_stats == 1:
                        ...
                        if self.adam_freeze_key and self.compress_mode == 0:
                            self.variance_norms.append(
                                torch.norm(exp_avg_sq_back).item())
                        else:
                    ...
                    if self.adam_freeze_key:
                        if self.compress_mode == 0:
                            ...
                            if ratio > state['last_ratio'] * (1.0 +
                                                              self.ratio_threshold):
                                ratio = state['last_ratio'] * (1.0 +
                                                               self.ratio_threshold)
                            if ratio < state['last_ratio'] * (1.0 -
                                                              self.ratio_threshold):
                                ratio = state['last_ratio'] * (1.0 -
