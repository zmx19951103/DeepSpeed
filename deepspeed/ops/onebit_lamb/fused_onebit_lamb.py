...
class FusedOnebitLamb(torch.optim.Optimizer):
    """Implements the LAMB algorithm with 1-bit compression. Currently GPU-only.
    ...
    Arguments:
        ...
        amsgrad (boolean, optional): NOT SUPPORTED in FusedOnebitLamb!
    """
    def __init__(self,
                 params,
                 lr=1e-3,
        ...
        try:
            fused_onebit_lamb_cuda = importlib.import_module(
                "deepspeed.ops.onebit_lamb.fused_onebit_lamb_cuda")
        except ImportError as err:
            print(
                "Unable to import 1-bit Lamb cuda extension, please build DeepSpeed with cuda/cpp extensions."
            )
            raise err

        if amsgrad:
            raise RuntimeError('FusedOnebitLamb does not support the AMSGrad variant.')
        defaults = dict(lr=lr,
    ...
    def step(self,
        ...
        for group, grads_this_group, output_params_this_group, grad_norm_group in zip(self.param_groups, grads_group, output_params_group, grad_norms):
            ...
            for p, grad, output_param, grad_norm in zip(group['params'], grads_this_group, output_params_this_group, grad_norm_group):
                ...
                if grad.is_sparse:
                    raise RuntimeError(
                        'FusedOnebitLamb does not support sparse gradients')
